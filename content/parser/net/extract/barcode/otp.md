...
steps:
    enable: true
    block:
    - title_left: "How to Extract Barcodes from OTP Documents via C# .NET "
      content_left: |
       GroupDocs.Parser .NET API helps software developers to extract Barcodes from OTP documents with ease. The following C# .NET code example demonstrates how to extract barcodes from a OTP document. 
    ...
    - title_left: "Barcodes Extraction from OTP Document's Page via .NET"
      content_left: |
       GroupDocs.Parser .NET enables software programmers to extract barcodes from OTP documents's page. The below C# .NET code shows how barcodes extraction can be achived  inside a OTP document. 
    ...
    - title_left: "Get Barcodes from OTP Document's Page Area via .NET"
      content_left: |
       GroupDocs.Parser .NET is a powerful  API that provides complete support for barcodes extraction from OTP documents using a couple of lines of .NET code. The following .NET code example shows how to perform barcodes extraction from a OTP document page area.

      title_right: "Extract Barcodes from OTP Page Area "
      content_right: |
        * Create an instance of [Parser](https://apireference.groupdocs.com/parser/net/groupdocs.parser/parser)   
        * Check document for barcodes extraction support

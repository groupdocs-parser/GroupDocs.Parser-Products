...
steps:
    enable: true
    block:
    - title_left: "How to Extract Barcodes from ODP Documents via C# .NET "
      content_left: |
       GroupDocs.Parser .NET API helps software developers to extract Barcodes from ODP documents with ease. The following C# .NET code example demonstrates how to extract barcodes from a ODP document. 
    ...
    - title_left: "Barcodes Extraction from ODP Document's Page via .NET"
      content_left: |
       GroupDocs.Parser .NET enables software programmers to extract barcodes from ODP documents's page. The below C# .NET code shows how barcodes extraction can be achived  inside a ODP document. 
    ...
    - title_left: "Get Barcodes from ODP Document's Page Area via .NET"
      content_left: |
       GroupDocs.Parser .NET is a powerful  API that provides complete support for barcodes extraction from ODP documents using a couple of lines of .NET code. The following .NET code example shows how to perform barcodes extraction from a ODP document page area.

      title_right: "Extract Barcodes from ODP Page Area "
      content_right: |
        * Create an instance of [Parser](https://apireference.groupdocs.com/parser/net/groupdocs.parser/parser)   
        * Check document for barcodes extraction support

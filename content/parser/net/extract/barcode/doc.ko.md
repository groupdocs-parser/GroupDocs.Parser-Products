...
about:
    enable: true
    title: "DOC 파일 .NET API에서 바코드를 추출하는 방법은 무엇입니까?"
    content: |
...
steps:
    enable: true
    title_left: ".NET의 DOC에서 바코드 추출"
    content_left: |
        [GroupDocs.Parser for .NET](/ko/parser/net/)를 사용하면 C# 개발자가 몇 가지 간단한 단계를 구현하여 DOC 파일에서 바코드를 쉽게 추출할 수 있습니다.
    ...
    code: |
     ...
     {{< parser/code-parser title="C# 예제 코드를 사용하여 DOC 파일에서 바코드를 추출하는 방법">}}

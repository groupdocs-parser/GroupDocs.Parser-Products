...
about:
    enable: true
    title: "OST ファイルからバーコードを抽出する方法 .NET API?"
    content: |
...
steps:
    enable: true
    title_left: ".NET の OST からバーコードを抽出します"
    content_left: |
        [GroupDocs.Parser for .NET](/ja/parser/net/) を使用すると、C# 開発者は、いくつかの簡単な手順を実装することで、OST ファイルからバーコードを簡単に抽出できます。
    ...
    code: |
     ...
     {{< parser/code-parser title="C# サンプルコードを使用して OST ファイルからバーコードを抽出する方法">}}

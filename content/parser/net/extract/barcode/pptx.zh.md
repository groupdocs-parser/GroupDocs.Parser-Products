...
draft: false
path: "zh/parser/net/extract/barcode//pptx/"
...
steps:
    enable: true
    block:
    - title_left: "如何通过 C# .NET 从 PPTX 文档中提取条形码 "
      content_left: |
       GroupDocs.Parser .NET API 帮助软件开发人员轻松地从 PPTX 文档中提取条形码。 以下 C# .NET 代码示例演示了如何从 PPTX 文档中提取条形码。
    ...
    - title_left: "通过 .NET 从 PPTX 文档页面提取条形码"
      content_left: |
       GroupDocs.Parser .NET 使软件程序员能够从 PPTX 文档的页面中提取条形码。 下面的 C# .NET 代码显示了如何在 PPTX 文档中实现条码提取。 
    ...
    - title_left: "通过 .NET 从 PPTX 文档的页面区域获取条形码"
      content_left: |
       GroupDocs.Parser .NET 是一个强大的 API，它为使用几行 .NET 代码从 PPTX 文档中提取条形码提供了完整的支持。 以下 .NET 代码示例显示了如何从 PPTX 文档页面区域执行条形码提取。

      title_right: "从 PPTX 页面区域提取条形码"
      content_right: |
        * 创建 [Parser](https://apireference.groupdocs.com/parser/net/groupdocs.parser/parser) 的实例
        * 检查文档以获取条码提取支持
        * 创建可用于条码提取的自定义选项

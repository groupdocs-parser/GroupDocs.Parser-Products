...
about:
    enable: true
    title: "如何通过 .NET API 从 XLS 文件中提取表？"
    content: |
...
steps:
    enable: true
    title_left: "从 .NET 中的 XLS 中提取表"
    content_left: |
        [GroupDocs.Parser for .NET](/zh/parser/net/) 让 C# 开发者只需执行几个简单的步骤即可轻松从 XLS 文件中提取表。
    ...
    code: |
     ...
     {{< parser/code-parser title="如何使用 C# 示例代码从 XLS 文件中提取表">}}

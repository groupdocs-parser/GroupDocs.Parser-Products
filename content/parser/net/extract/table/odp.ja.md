...
about:
    enable: true
    title: ".NET API 経由で ODP ファイルからテーブルを抽出するにはどうすればよいですか?"
    content: |
...
steps:
    enable: true
    title_left: ".NET の ODP からテーブルを抽出します"
    content_left: |
        [GroupDocs.Parser for .NET](/ja/parser/net/) を使用すると、C# 開発者は、いくつかの簡単な手順を実装することで、ODP ファイルからテーブルを簡単に抽出できます。
    ...
    code: |
     ...
     {{< parser/code-parser title="C# サンプルコードを使用して ODP ファイルからテーブルを抽出する方法">}}

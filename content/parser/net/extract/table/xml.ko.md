...
about:
    enable: true
    title: ".NET API를 통해 XML 파일에서 테이블을 추출하는 방법은 무엇입니까?"
    content: |
...
steps:
    enable: true
    title_left: ".NET의 XML에서 테이블 추출"
    content_left: |
        [GroupDocs.Parser for .NET](/ko/parser/net/)를 사용하면 C# 개발자가 몇 가지 간단한 단계를 구현하여 XML 파일에서 테이블을 쉽게 추출할 수 있습니다.
    ...
    code: |
     ...
     {{< parser/code-parser title="C# 예제 코드를 사용하여 XML 파일에서 테이블을 추출하는 방법">}}

...
draft: false
path: "ja/parser/net/extract/table/xlsb/"
...
steps:
    enable: true
    block:
    - title_left: "C＃.NETを介して XLSBドキュメントからテーブルを抽出する方法 "
      content_left: |
       GroupDocs.Parser .NET APIは、ソフトウェア開発者がわずか数行のコードで XLSB ドキュメントからテーブルを抽出するのに役立ちます。 次のC＃.NETコード例は、開発者が XLSBドキュメントからテーブルを抽出する方法を示しています。 
    ...
    - title_left: ".NET APIを使用して、XLSB ドキュメントのページからテーブルを抽出します"
      content_left: |
       GroupDocs.Parser .NETを使用すると、ソフトウェア開発者は XLSBドキュメントのページからテーブルを抽出できます。 次のC＃.NETコードは、プログラマーがXLSBドキュメント内でバーコード抽出を実行する方法を示しています。。

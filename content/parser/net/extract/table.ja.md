...
about:
    enable: true
    title: ".NET API を介して Excel、Word、PDF、およびその他のドキュメントからテーブルを抽出するにはどうすればよいですか?"
    content: |
...
steps:
    enable: true
    title_left: ".NET のドキュメントからテーブルを抽出する"
    content_left: |
        [GroupDocs.Parser for .NET](/ja/parser/net/) を使用すると、C# 開発者はいくつかの簡単な手順を実装することで、ドキュメントからテーブルを簡単に抽出できます。
    ...
    code: |
     ...
     {{< parser/code-parser title="C# サンプルコードを使用してドキュメントから表を抽出する方法">}}

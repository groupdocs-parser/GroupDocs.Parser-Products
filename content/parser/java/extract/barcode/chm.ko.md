...
about:
    enable: true
    title: "CHM 파일 Java API에서 바코드를 추출하는 방법은 무엇입니까?"
    content: |
...
steps:
    enable: true
    title_left: "Java의 CHM에서 바코드 추출"
    content_left: |
        [GroupDocs.Parser for Java](/ko/parser/java/)를 사용하면 Java 개발자가 몇 가지 간단한 단계를 구현하여 CHM 파일에서 바코드를 쉽게 추출할 수 있습니다.
    ...
    code: |
     ...
     {{< parser/code-parser title="Java 예제 코드를 사용하여 CHM 파일에서 바코드를 추출하는 방법">}}

        ```java    
        // GroupDocs.Parser API를 사용하여 CHM 파일에서 바코드 추출
        // Parser 클래스의 인스턴스 생성
...
demos:
    enable: true
    title: "라이브 데모 - CHM 온라인에서 바코드 추출"
    content: |
       지금 바로 [GroupDocs.Parser Live Demos](https://products.groupdocs.app/parser/barcodes/chm) 웹사이트를 방문하여 CHM 파일에서 바코드를 추출하세요.

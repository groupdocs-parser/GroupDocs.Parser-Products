...
about:
    enable: true
    title: "ODP ファイルからバーコードを抽出する方法 Java API?"
    content: |
...
steps:
    enable: true
    title_left: "Java の ODP からバーコードを抽出します"
    content_left: |
        [GroupDocs.Parser for Java](/ja/parser/java/) を使用すると、Java 開発者は、いくつかの簡単な手順を実装することで、ODP ファイルからバーコードを簡単に抽出できます。
    ...
    code: |
     ...
     {{< parser/code-parser title="Java サンプルコードを使用して ODP ファイルからバーコードを抽出する方法">}}

        ```java    
        // GroupDocs.Parser API を使用して ODP ファイルからバーコードを抽出します
...
demos:
    enable: true
    title: "ライブ デモ - オンライン ODP からバーコードを抽出"
    content: |
       [GroupDocs.Parser ライブ デモ](https://products.groupdocs.app/parser/barcodes/odp) Web サイトにアクセスして、今すぐ ODP ファイルからバーコードを抽出してください。

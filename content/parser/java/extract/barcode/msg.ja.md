...
about:
    enable: true
    title: "MSG ファイルからバーコードを抽出する方法 Java API?"
    content: |
...
steps:
    enable: true
    title_left: "Java の MSG からバーコードを抽出します"
    content_left: |
        [GroupDocs.Parser for Java](/ja/parser/java/) を使用すると、Java 開発者は、いくつかの簡単な手順を実装することで、MSG ファイルからバーコードを簡単に抽出できます。
    ...
    code: |
     ...
     {{< parser/code-parser title="Java サンプルコードを使用して MSG ファイルからバーコードを抽出する方法">}}

        ```java    
        // GroupDocs.Parser API を使用して MSG ファイルからバーコードを抽出します
...
demos:
    enable: true
    title: "ライブ デモ - オンライン MSG からバーコードを抽出"
    content: |
       [GroupDocs.Parser ライブ デモ](https://products.groupdocs.app/parser/barcodes/msg) Web サイトにアクセスして、今すぐ MSG ファイルからバーコードを抽出してください。

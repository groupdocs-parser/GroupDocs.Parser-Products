...
steps:
    enable: true
    title_left: "从Java中的DOCX中提取条形码"
    content_left: |
        [GroupDocs.Parser for Java](/zh/parser/java/) 让 Java 开发者只需执行几个简单的步骤即可轻松从 DOCX 文件中提取条形码。
    ...
    code: |
     ...
     {{< parser/code-parser title="如何使用 Java 示例代码从 DOCX 文件中提取条形码">}}

        ```java    
        // 使用 GroupDocs.Parser API 从 DOCX 文件中提取条形码
        // 创建 Parser 类的实例
...
demos:
    enable: true
    title: "现场演示 - 从 DOCX 在线提取条形码"
    content: |
       立即访问 [GroupDocs.Parser 现场演示](https://products.groupdocs.app/parser/barcodes/docx) 网站，从 DOCX 文件中提取条形码。
       现场演示有以下好处。

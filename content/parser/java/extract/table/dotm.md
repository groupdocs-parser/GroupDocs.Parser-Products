...
about:
    enable: true
    title: "How to Extract Tables from DOTM files via Java API?"
    content: |
...
steps:
    enable: true
    title_left: "Extract tables from DOTM in Java"
    content_left: |
        [GroupDocs.Parser for Java](/parser/java/) makes it easy for Java developers to extract tables from a DOTM file by implementing a few easy steps.
    ...
    code: |
     ...
     {{< parser/code-parser title="How to extract tables from DOTM file using Java example code">}}

        ```java    
        // Extract tables from DOTM file using GroupDocs.Parser API

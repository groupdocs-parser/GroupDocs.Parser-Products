...
draft: false
path: "de/parser/java/extract/table/xla/"
...
steps:
    enable: true
    block:
    - title_left: "Verwenden Sie Java-Code, um Tabellen aus XLA-Dokumenten zu extrahieren "
      content_left: |
       GroupDocs.Parser Java API bietet vollständige Unterstützung für die Verarbeitung verschiedener Dokumenttypen und das Extrahieren von Daten daraus. Das folgende Java-Codebeispiel zeigt, wie Softwareprogrammierer mit nur wenigen Codezeilen Tabellen aus einem XLA-Dokument extrahieren können. 

      title_right: "Tabellenextraktion aus XLA Dokumenten"
      content_right: |
    ...
    - title_left: "So extrahieren Sie Tabellen aus der Seite des XLA-Dokuments"
      content_left: |
       GroupDocs.Parser Java API ermöglicht Computerprogrammierern das Extrahieren von Tabellen aus der Seite des Dokuments XLA mit nur ein paar Zeilen Java-Code. Es überprüft das Dokument auf das Vorhandensein von Tabellen und extrahiert dann Tabellen aus bestimmten Dokumentenseiten. Das folgende Beispiel zeigt, wie Java-Entwickler mühelos eine Tabellenextraktion innerhalb eines XLA-Dokuments durchführen können.  

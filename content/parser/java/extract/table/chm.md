...
about:
    enable: true
    title: "How to Extract Tables from CHM files via Java API?"
    content: |
...
steps:
    enable: true
    title_left: "Extract tables from CHM in Java"
    content_left: |
        [GroupDocs.Parser for Java](/parser/java/) makes it easy for Java developers to extract tables from a CHM file by implementing a few easy steps.
    ...
    code: |
     ...
     {{< parser/code-parser title="How to extract tables from CHM file using Java example code">}}

        ```java    
        // Extract tables from CHM file using GroupDocs.Parser API

...
about:
    enable: true
    title: "How to Extract Tables from PPSX files via Java API?"
    content: |
...
steps:
    enable: true
    title_left: "Extract tables from PPSX in Java"
    content_left: |
        [GroupDocs.Parser for Java](/parser/java/) makes it easy for Java developers to extract tables from a PPSX file by implementing a few easy steps.
    ...
    code: |
     ...
     {{< parser/code-parser title="How to extract tables from PPSX file using Java example code">}}

        ```java    
        // Extract tables from PPSX file using GroupDocs.Parser API

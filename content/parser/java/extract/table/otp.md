...
about:
    enable: true
    title: "How to Extract Tables from OTP files via Java API?"
    content: |
...
steps:
    enable: true
    title_left: "Extract tables from OTP in Java"
    content_left: |
        [GroupDocs.Parser for Java](/parser/java/) makes it easy for Java developers to extract tables from a OTP file by implementing a few easy steps.
    ...
    code: |
     ...
     {{< parser/code-parser title="How to extract tables from OTP file using Java example code">}}

        ```java    
        // Extract tables from OTP file using GroupDocs.Parser API

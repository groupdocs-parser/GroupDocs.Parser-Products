...
about:
    enable: true
    title: "How to Extract Tables from DOCX files via Java API?"
    content: |
...
steps:
    enable: true
    title_left: "Extract tables from DOCX in Java"
    content_left: |
        [GroupDocs.Parser for Java](/parser/java/) makes it easy for Java developers to extract tables from a DOCX file by implementing a few easy steps.
    ...
    code: |
     ...
     {{< parser/code-parser title="How to extract tables from DOCX file using Java example code">}}

        ```java    
        // Extract tables from DOCX file using GroupDocs.Parser API

...
draft: false
path: "ru/parser/java/extract/table/ppt/"
...
steps:
    enable: true
    block:
    - title_left: "Используйте код Java для извлечения таблиц из PPT документов "
      content_left: |
       GroupDocs.Parser Java API включает полную поддержку обработки различных типов документов и извлечения из них данных. В следующем примере кода Java показано, как программисты могут извлекать таблицы из документа PPT, написав всего пару строк кода.

      title_right: "Извлечение таблиц из PPT документов"
      content_right: |

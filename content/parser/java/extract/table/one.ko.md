...
about:
    enable: true
    title: "Java API를 통해 ONE 파일에서 테이블을 추출하는 방법은 무엇입니까?"
    content: |
...
steps:
    enable: true
    title_left: "Java의 ONE에서 테이블 추출"
    content_left: |
        [GroupDocs.Parser for Java](/ko/parser/java/)를 사용하면 Java 개발자가 몇 가지 간단한 단계를 구현하여 ONE 파일에서 테이블을 쉽게 추출할 수 있습니다.
    ...
    code: |
     ...
     {{< parser/code-parser title="Java 예제 코드를 사용하여 ONE 파일에서 테이블을 추출하는 방법">}}

        ```java    
        // GroupDocs.Parser API를 사용하여 ONE 파일에서 테이블 추출
        // Parser 클래스의 인스턴스 생성

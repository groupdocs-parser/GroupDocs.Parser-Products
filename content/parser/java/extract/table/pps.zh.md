...
about:
    enable: true
    title: "如何通过 Java API 从 PPS 文件中提取表？"
    content: |
...
steps:
    enable: true
    title_left: "从 Java 中的 PPS 中提取表"
    content_left: |
        [GroupDocs.Parser for Java](/zh/parser/java/) 让 Java 开发者只需执行几个简单的步骤即可轻松从 PPS 文件中提取表。
    ...
    code: |
     ...
     {{< parser/code-parser title="如何使用 Java 示例代码从 PPS 文件中提取表">}}

        ```java    
        // 使用 GroupDocs.Parser API 从 PPS 文件中提取表
        // 创建 Parser 类的实例

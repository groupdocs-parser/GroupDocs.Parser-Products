...
draft: false
path: "parser/java/extract/table/ott/"
...
steps:
    enable: true
    block:
    - title_left: "Use Java Code to Extract Tables from OTT Documents "
      content_left: |
       GroupDocs.Parser Java API has included complete support for processing various documents types and extract data from it. The following Java code example shows how software programmers can extract tables from a OTT document with just couple of lines of code. 

      title_right: "Tables Extraction from OTT Documents"
      content_right: |
    ...
    - title_left: "How to Extract Tables from OTT Document's Page"
      content_left: |
       GroupDocs.Parser Java API allows computer programmers to extract tables from OTT document's page with just a couple of lines of Java code. It will check document for tables existence and then will extract tables from particular documents page. The following example demonstrates how Java developers can perform tables extraction inside a OTT document with ease.  

...
draft: false
path: "ru/parser/java/extract/table/ost/"
...
steps:
    enable: true
    block:
    - title_left: "Используйте код Java для извлечения таблиц из OST документов "
      content_left: |
       GroupDocs.Parser Java API включает полную поддержку обработки различных типов документов и извлечения из них данных. В следующем примере кода Java показано, как программисты могут извлекать таблицы из документа OST, написав всего пару строк кода.

      title_right: "Извлечение таблиц из OST документов"
      content_right: |

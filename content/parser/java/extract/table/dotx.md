...
about:
    enable: true
    title: "How to Extract Tables from DOTX files via Java API?"
    content: |
...
steps:
    enable: true
    title_left: "Extract tables from DOTX in Java"
    content_left: |
        [GroupDocs.Parser for Java](/parser/java/) makes it easy for Java developers to extract tables from a DOTX file by implementing a few easy steps.
    ...
    code: |
     ...
     {{< parser/code-parser title="How to extract tables from DOTX file using Java example code">}}

        ```java    
        // Extract tables from DOTX file using GroupDocs.Parser API

...
draft: false
path: "de/parser/java/extract/table/xlsx/"
...
steps:
    enable: true
    block:
    - title_left: "Verwenden Sie Java-Code, um Tabellen aus XLSX-Dokumenten zu extrahieren "
      content_left: |
       GroupDocs.Parser Java API bietet vollständige Unterstützung für die Verarbeitung verschiedener Dokumenttypen und das Extrahieren von Daten daraus. Das folgende Java-Codebeispiel zeigt, wie Softwareprogrammierer mit nur wenigen Codezeilen Tabellen aus einem XLSX-Dokument extrahieren können. 

      title_right: "Tabellenextraktion aus XLSX Dokumenten"
      content_right: |
    ...
    - title_left: "So extrahieren Sie Tabellen aus der Seite des XLSX-Dokuments"
      content_left: |
       GroupDocs.Parser Java API ermöglicht Computerprogrammierern das Extrahieren von Tabellen aus der Seite des Dokuments XLSX mit nur ein paar Zeilen Java-Code. Es überprüft das Dokument auf das Vorhandensein von Tabellen und extrahiert dann Tabellen aus bestimmten Dokumentenseiten. Das folgende Beispiel zeigt, wie Java-Entwickler mühelos eine Tabellenextraktion innerhalb eines XLSX-Dokuments durchführen können.  

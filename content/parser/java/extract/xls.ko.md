...
draft: false
path: "ko/parser/java/extract/xls/"
...
steps:
    enable: true
    block:
    - title_left: "XLS 문서에서 하이퍼링크를 추출하는 방법"
      content_left: |
       GroupDocs.Parser Java에는 XLS 문서에서 하이퍼링크를 추출하는 기능이 포함되어 있습니다. 다음 Java 코드 예제는 XLS 문서에서 하이퍼링크를 추출하는 방법을 보여줍니다. 
    ...
    - title_left: "XLS 문서 페이지에서 하이퍼링크를 추출하는 방법"
      content_left: |
       GroupDocs.Parser .NET을 사용하면 소프트웨어 개발자가 몇 줄의 코드로 XLS 문서에서 하이퍼링크를 추출할 수 있습니다. 아래 C# .NET 코드는 XLS 문서 내 하이퍼링크 추출을 보여줍니다. 
    ...
    - title_left: "XLS 문서 페이지 영역에서 하이퍼링크 추출"
      content_left: |
       GroupDocs.Parser Java API는 XLS 문서의 페이지 용이성에서 하이퍼링크를 추출하기 위한 완전한 지원을 제공했습니다. 다음 Java 코드는 프로그래머가 자체 Java 애플리케이션 내부의 XLS 문서 페이지 영역에서 하이퍼링크를 추출하는 방법을 보여줍니다.

...
about:
    enable: true
    title: "Java API を介して Excel、Word、PDF、およびその他のドキュメントからテーブルを抽出するにはどうすればよいですか?"
    content: |
...
steps:
    enable: true
    title_left: "Java のドキュメントからテーブルを抽出する"
    content_left: |
        [GroupDocs.Parser for Java](/ja/parser/java/) を使用すると、Java 開発者はいくつかの簡単な手順を実装することで、ドキュメントからテーブルを簡単に抽出できます。
    ...
    code: |
     ...
     {{< parser/code-parser title="Java サンプルコードを使用してドキュメントから表を抽出する方法">}}

        ```java    
        // GroupDocs.Parser API を使用してドキュメントからテーブルを抽出する

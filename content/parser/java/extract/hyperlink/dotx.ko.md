...
about:
    enable: true
    title: "Java API를 통해 DOTX 문서에서 하이퍼링크를 구문 분석하고 추출하는 방법은 무엇입니까?"
    content: |
...
steps:
    enable: true
    title_left: "Java의 DOTX에서 하이퍼링크 추출"
    content_left: |
        [GroupDocs.Parser for Java](/ko/parser/java/)를 사용하면 Java 개발자가 몇 가지 간단한 단계를 구현하여 DOTX 파일에서 하이퍼링크를 쉽게 추출할 수 있습니다.
    ...
    code: |
     ...
     {{< parser/code-parser title="Java 예제 코드를 사용하여 DOTX 파일에서 하이퍼링크를 추출하는 방법">}}

        ```java    
        // GroupDocs.Parser API를 사용하여 DOTX 파일에서 하이퍼링크 추출
        // Parser 클래스의 인스턴스 생성

...
about:
    enable: true
    title: "How to Parse & Extract Hyperlinks from XLS documents via Java API?"
    content: |
...
steps:
    enable: true
    title_left: "Extract hyperlinks from XLS in Java"
    content_left: |
        [GroupDocs.Parser for Java](/parser/java/) makes it easy for Java developers to extract hyperlinks from a XLS file by implementing a few easy steps.
    ...
    code: |
     ...
     {{< parser/code-parser title="How to extract hyperlinks from XLS file using Java example code">}}

        ```java    
        // Extract hyperlinks from XLS file using GroupDocs.Parser API

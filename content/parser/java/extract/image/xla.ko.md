...
draft: false
path: "ko/parser/java/extract/image/xla/"
...
steps:
    enable: true
    block:
    - title_left: "XLA 문서에서 이미지를 추출하는 방법"
      content_left: |
       GroupDocs.Parser Java에는 XLA 문서에서 이미지를 추출하는 기능이 포함되어 있습니다. 다음 Java 코드 예제는 XLA 문서에서 이미지를 쉽게 추출하는 방법을 보여줍니다. 
    ...
    - title_left: "XLA 문서 페이지에서 이미지 추출"
      content_left: |
       GroupDocs.Parser Java API를 사용하면 소프트웨어 개발자가 몇 줄의 코드로 XLA 문서에서 이미지를 추출할 수 있습니다. 아래 Java 코드는 XLA 문서에서 이미지 추출을 보여줍니다.
    ...
    - title_left: "XLA 문서 페이지 영역에서 이미지를 추출하는 방법"
      content_left: |
       GroupDocs.Parser Java API는 XLA 문서의 페이지 용이성에서 추출을 위한 완벽한 지원을 제공했습니다. 다음 Java 코드는 프로그래머가 자체 Java 앱 내부의 XLA 문서 페이지 영역에서 이미지를 추출하는 방법을 보여줍니다.
    ...
    - title_left: "Java API를 통해 이미지를 파일로 추출하는 방법"
      content_left: |
       GroupDocs.Parser Java API를 사용하면 XLA 문서에서 이미지를 추출하고 이미지 내용을 파일에 저장할 수 있습니다. 다음 Java 코드는 프로그래머가 자신의 Java 앱 내에서 선택한 파일로 이미지를 추출하는 방법을 보여줍니다.
